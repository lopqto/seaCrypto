...
size_t base64_decode_size(const char *input)
{
	...
	if (input[inputlen - 1] == '=') decode_size--;
    if (input[inputlen - 2] == '=') decode_size--;
...
unsigned char *base64_decode(const char *input, size_t inputlen)
{
    ...
    if (base64_decode_table == NULL) base64_generate_decode_table();
    ...
    for (size_t i = 0, j = 0; i < inputlen;) 
    {
        ...
        if (j < decode_size) output[j++] = (concat >> 2 * 8) & 0xFF;
        if (j < decode_size) output[j++] = (concat >> 1 * 8) & 0xFF;
        if (j < decode_size) output[j++] = (concat >> 0 * 8) & 0xFF;
    }
